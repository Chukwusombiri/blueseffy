...
<div class="card">
    <div class="card-header">
      <h3 class="card-title">Investments Management</h3>
    ...
    <div class="card-body table-responsive">          
      <div class="input-group input-group-sm mb-2" style="width: 150px;">
        <a href="{{route('admin.investment.create')}}" class="btn btn-block btn-primary btn-sm">New investment</a>
      </div> 
        ...
        <tbody>
          @if (count($investments)>0)
            @foreach ($investments as $investment)
              <tr>                           
                <td>{{$investment->user->name}}<a href="{{route('admin.user.show',[$investment->user_id])}}" class="ml-3 underline">See more</a></td>
                @if($investment->created_at > auth()->user()->last_sign_out_at)
