...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">Deposit Management</h3>
    <div class="card-tools">
      <div class="input-group input-group-sm" style="width: 300px;">
        <input type="text" class="form-control float-right border-none" style="background-color:#edf2f7 "
        placeholder="filter by name or amount or plan or wallet" wire:model="search">
        @if ($search!=='')
  ...
  <div class="card-body table-responsive">          
    <div class="input-group input-group-sm mb-2" style="width: 150px;">
      <a href="{{route('admin.deposit.create')}}" class="btn btn-block btn-primary btn-sm">New Deposit</a>
    </div> 
    ...
    <table class="table table-hover table-head-fixed  text-nowrap">
      ...
      <tbody>
        @if (count($deposits)>0)
          @foreach ($deposits as $deposit)
            <tr>                           
              <td>{{$deposit->user->name}}<a href="{{route('admin.user.show',[$deposit->user_id])}}" class="ml-3 underline">See more</a></td>
              @if($deposit->created_at > auth()->user()->last_sign_out_at)
                <td>${{$deposit->amount}}
              ...
              <td class="flex flex-nowrap justify-center items-center">
                <div class="flex items-center">
                  <button type="button" data-toggle="dropdown" aria-expanded="false" class="bg-gray-500 bg-opacity-50 p-[8px] rounded">
                  ...
                  <ul class="dropdown-menu left-4"> 
                    @if (!$deposit->is_approved)
                        <li><button wire:click="approve('{{$deposit->id}}')" type="button" class="dropdown-item flex justify-start items-center">
                    ...
                    <li><a href="{{ asset('storage/'.$deposit->receipt) }}" target="_blank" class="dropdown-item flex justify-start items-center">
                      <span class="mr-1"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    @endif                       
                    <li><button wire:click='$emit("openModal","admin.edit-deposit",@json([$deposit->id]))'
                    ...
                    <li><button wire:click="delete('{{$deposit->id}}')" type="button" class="dropdown-item flex justify-start items-center">
                      <span class="mr-1"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
      ...
      <div class="p-2">{{$deposits->links()}}</div>
    </table>
  </div>
  <!-- /.card-body -->
</div>