<div class="mycontainer">
    <div class="mb-20">
        <span class="flex flex-nowrap justify-center item-center text-base text-indigo-600 uppercase text-extrabold mb-7">Make Investment</span>
        <h2 class="text-center text-dark text-2xl capitalize mb-4 mx-auto md:w-1/2">Fill out your investment form below.</h2>
        <p class="text-center mx-auto text-slate-600 md:w-1/2"> BLUESTECH accepts only cryptocurrency as a source of payment for investment. 
            This policy has given a 10X boost to the speed at which transactions
            are processed thereby protecting our members against market slippage.
            Investment can also be funded using available funds in your dormant funds wallet
        </p>
    </div>                                          
        ...
        <div class="col-span-2 md:col-span-1">                 
            ...
            <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6 md:col-span-5 flex flex-wrap justify-between">                    
                        <label for="crypto">
                            <x-jet-input type="radio" id="crypto" wire:model="source" value="crypto"/>   
                            CRYPTOCURRENCY
                        </label>
                        
                        <label for="funds" value="ACCOUNT FUNDS">
                            <x-jet-input id="funds" type="radio" wire:model="source" value="funds" />
                            ACCOUNT FUNDS
                        </label>                                                                                            
                    </div>                       
                    @if ($source==='funds')
                    <div class="col-span-6 md:col-span-5">                    
                        <p class="text-lg font-bold text-slate-600 py-7">Available Funds: ${{$user->doBal}}</p>
                    </div>   
                    @endif
                    
                    <div class="col-span-6 md:col-span-5">                    
                        <x-jet-label for="amount" value="{{ 'Amount: Range($'.$plan->min.' - $'.$plan->max.')' }}" />
                    ...
                    <div class="col-span-6 sm:col-span-5">
                        ...
                        <select name="wallet_id" id="wallet_id" class="mt-1 block w-full 
                        border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 
                        focus:ring-opacity-50 rounded-md shadow-sm" wire:model.defer="wallet_id">
                        <option value="" @if(old('wallet_id')==''){{'selected'}}@endif>select cryptocurrency</option>                    
                        @foreach ($wallets as $wallet)
                            <option value="{{$wallet->id}}"  @if(old('wallet_id')==$wallet->id){{'selected'}}@endif>
                                 {{$wallet->name}}</option>                           
                        @endforeach
                        </select>            
...
<section class="bg-gray-100 py-12 px-2 md:px-8">
    <div class="container mx-auto">
        <h2 class="text-3xl font-semibold mb-6 text-center">Our Trusted Partners</h2>
        <p class="max-w-xl text-md mx-auto mb-4">Do you want to purchase crypto to kick start your investment on {{config('app.name')}}? Our trusted partners makes it a breeze!</p>
