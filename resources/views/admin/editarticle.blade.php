        ...
        <div class="card card-primary">
            ...
            <form method="POST" action="{{route('admin.articles.update',[$article->id])}}" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  ...
                  <div class="form-group col-12">
                    <label for="author_comment">Author Comment(optional)</label>
                    <input type="text" class="form-control @error('author_comment') is-invalid @enderror" id="author_comment" name="author_comment" value="{{old('author_comment') ?? $article->author_comment}}">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <select id="category_id" name="category_id" class="form-control @error('category_id') is-invalid @enderror" required>
                      <option @if(old('category_id')=='' || $article->category_id ==''){{'selected'}}@endif>Choose...</option>
                      @if (count($categories)>0)
                        @foreach ($categories as $category)
                            <option value="{{$category->id}}" @if(old('category_id')==$category->id || $article->category_id ==$category->id){{'selected'}}@endif>{{$category->name}}</option>    
                        @endforeach                                             
                      @else
                  ...
                  <div class="form-group">
                    <label for="author_img" (optional)>Upload Author image</label>
