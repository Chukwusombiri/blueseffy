...
class AdminDashBoardController extends Controller
{
    ...
    public function index()
    {
        ...
        $newinvestments = InvestmentDeposit::whereHas('user')
            ->with('user')
            ->where('created_at', '>', auth()->user()->last_sign_out_at)
            ->get();

        $wits = Withdrawal::whereHas('user')
            ->with('user')
            ->where('created_at', '>', auth()->user()->last_sign_out_at)
            ->where('is_verified', 1)
            ->get();

        $fiat_wits = FiatWithdrawal::whereHas('user')
            ->with('user')
            ->where('created_at', '>', auth()->user()->last_sign_out_at)
            ->where('is_verified', 1)
            ->get();
        ...
        return view('admin.dashboard')->with([
            ...
            'dobal' => $dobal,
            'totbal' => $totbal,
            'totinvestmentcount' => $totinvestmentcount,
            'investments' => $investments,
            'invested_count' => $invested_count,
            'totdepcount' => $totdepcount,
            'totwitcount' => $totwitcount,
            'totrefcount' => $totrefcount,
            'invested_value' => $invested_value,
            'totpromocount' => $totpromocount,
            'newusers' => $newusers,
            ...
            'totfiatwitcount' => $totfiatwitcount,
        ]);
    }
